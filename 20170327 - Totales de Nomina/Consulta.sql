SELECT /*+ LEADING(PPF, PPA, PTP, PAA, PRR, PRRV) */ 
       ROW_NUMBER() OVER (PARTITION 
                                 BY 1
                              ORDER 
                                 BY SUBSTR(PETF.ELEMENT_NAME,0,1) DESC,
                                    PETF.ELEMENT_NAME,
                                    PPF.ATTRIBUTE1) 
                                    AS  ROWINDEX,
 ...
 GROUP 
    BY PETF.ELEMENT_NAME,
       PPF.ATTRIBUTE1
UNION
SELECT /*+ LEADING(PPF, PPA, PTP, PPG, PAA, PRR, PRRV) */
       1111                         AS  ROWINDEX, 
 ...
 GROUP 
    BY PPF.ATTRIBUTE1
UNION
SELECT /*+ LEADING(PAAF, PPF, PPA, PTP, PPG, PAA, PRR, PRRV) */
       2222 + ROW_NUMBER() OVER (PARTITION 
                                 BY 1
                              ORDER 
                                 BY POPM.ORG_PAYMENT_METHOD_NAME,
                                    PPF.ATTRIBUTE1) 
                                    AS  ROWINDEX,
