/* Formatted on 22/05/2015 02:21:15 p.m. (QP5 v5.163.1008.3004) */
SELECT 
       DETAIL.ANO,
       DETAIL.MES,
       DETAIL.PRODUCTO,
       DETAIL.ORIGEN,
--       DETAIL.CLIENTE,
       NVL(DETAIL.DOMICILIO, REPLACE(ORGANIZATION_ID, 'CEDIS_', '')) DESTINO,
       DETAIL.CANTIDAD_EMPAQUE ,
       (CASE
            WHEN DETAIL.CANTIDAD_EMPAQUE = '30 PIEZAS' OR DETAIL.CANTIDAD_EMPAQUE = '20 PIEZAS' THEN
                'MINORISTAS'
            ELSE
                'MAYORISTAS'
        END)    AS  TIPO,
...
--       DETAIL.CLIENTE,
--       DETAIL.CANAL,
       TO_CHAR(SUM(DETAIL.VOLUMEN_VENTA))  AS VOLUMEN_VENTA,
       DETAIL.UOM_CODE,
--       CANAL
        ORGANIZATION_ID
--       SUM(DETAIL.VALOR_TOTAL) AS VALOR_TOTAL
  FROM (
              SELECT /*RG.NAME,*/
                     CT.TRX_NUMBER FACTURA,
                     CTT.NAME,
