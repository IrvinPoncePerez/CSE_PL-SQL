...
/* ===== Latest balance creation begin ==== */
          ...
          SOE_ytd = D087_FINAN_OPTICA_ASG_GRE_YTD
          SOE_mtd = D087_FINAN_OPTICA_ASG_GRE_MTD
...
/* ====  Entry ITD Check Begin ==== */
   ...
   IF ( D087_FINAN_OPTICA_ACCRUED_ENTRY_ITD = 0 AND
        D087_FINAN_OPTICA_ACCRUED_ASG_GRE_ITD <> 0 ) THEN
   (
      to_total_owed = -1 * D087_FINAN_OPTICA_ACCRUED_ASG_GRE_ITD + dedn_amt
   )

   IF ( D087_FINAN_OPTICA_ARREARS_ENTRY_ITD = 0 AND
        D087_FINAN_OPTICA_ARREARS_ASG_GRE_ITD <> 0 ) THEN
   (
      to_arrears = -1 * D087_FINAN_OPTICA_ARREARS_ASG_GRE_ITD
   )
...
/* ===== Arrears Section Begin ===== */
   ...
   ELSE
   (
      IF D087_FINAN_OPTICA_ARREARS_ASG_GRE_ITD <> 0 THEN
      (
         to_arrears = -1 * D087_FINAN_OPTICA_ARREARS_ASG_GRE_ITD
      )
   )
   ...
   (
      ...
      ELSE
      (
         total_dedn = dedn_amt + D087_FINAN_OPTICA_ARREARS_ASG_GRE_ITD
         ...
         (
            ...
            (
               ...
               to_arrears   = to_arrears - D087_FINAN_OPTICA_ARREARS_ASG_GRE_ITD
...
/* ===== Stop Rule Section Begin ===== */
   ...
   (
      total_accrued  = dedn_amt + D087_FINAN_OPTICA_ACCRUED_ENTRY_ITD

      IF total_accrued  >= Total_Owed THEN
      (
         dedn_amt = Total_Owed - D087_FINAN_OPTICA_ACCRUED_ENTRY_ITD

          /* The total has been reached - the return will stop the entry under
             these conditions.  Also, zero out Accrued balance.  */

          to_total_owed = -1 * D087_FINAN_OPTICA_ACCRUED_ENTRY_ITD
          STOP_ENTRY = 'Y'

          mesg = GET_MESG('PAY','PAY_MX_STOPPED_ENTRY',
                                  'BASE_NAME','D087_FINAN_OPTICA')
       )
...
IF SALDO_ACUMULADO > 0  /*(D087_FINAN_OPTICA_ACCRUED_ASG_GRE_ITD <> 0 )*/ THEN
	(
	Saldo = SALDO_ACUMULADO  /*D087_FINAN_OPTICA_ACCRUED_ASG_GRE_ITD + dedn_amt*/
