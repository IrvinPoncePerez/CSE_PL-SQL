...
/* ===== Latest balance creation begin ==== */
          ...
          SOE_ytd = D065_LLAM_TELEFONICAS_ASG_GRE_YTD
          SOE_mtd = D065_LLAM_TELEFONICAS_ASG_GRE_MTD
...
/************************************ IPONCE  18-SEP-2015*/
IF (Futuro1 > 0 AND (trunc(dedn_amt, 0) + Futuro1 + 1) > Total_Owed) THEN (
  dedn_amt = Total_Owed - Futuro1
) 
/*************************************/   

/* ====  Entry ITD Check Begin ==== */

   IF ( D065_LLAM_TELEFONICAS_ACCRUED_ENTRY_ITD = 0 AND
        D065_LLAM_TELEFONICAS_ACCRUED_ASG_GRE_ITD <> 0 ) THEN
   (
      to_total_owed = -1 * D065_LLAM_TELEFONICAS_ACCRUED_ASG_GRE_ITD + dedn_amt
   )

   IF ( D065_LLAM_TELEFONICAS_ARREARS_ENTRY_ITD = 0 AND
        D065_LLAM_TELEFONICAS_ARREARS_ASG_GRE_ITD <> 0 ) THEN
   (
      to_arrears = -1 * D065_LLAM_TELEFONICAS_ARREARS_ASG_GRE_ITD
   )
...
/* ===== Arrears Section Begin ===== */
   ...
   ELSE
   (
      IF D065_LLAM_TELEFONICAS_ARREARS_ASG_GRE_ITD <> 0 THEN
      (
         to_arrears = -1 * D065_LLAM_TELEFONICAS_ARREARS_ASG_GRE_ITD
      )
   )
   ...
   (
      ...
      ELSE
      (
         total_dedn = dedn_amt + D065_LLAM_TELEFONICAS_ARREARS_ASG_GRE_ITD
         ...
         (
            ...
            (
               ...
               to_arrears   = to_arrears - D065_LLAM_TELEFONICAS_ARREARS_ASG_GRE_ITD
...
IF SALDO_ACUMULADO >  0  /*(D065_LLAM_TELEFONICAS_ACCRUED_ASG_GRE_ITD <> 0 )*/ THEN
	(
	Saldo = SALDO_ACUMULADO /* D065_LLAM_TELEFONICAS_ACCRUED_ASG_GRE_ITD + dedn_amt*/
