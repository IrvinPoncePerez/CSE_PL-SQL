...
----             CAMBIO SOLICITADO POR EL SR #228736
           ...
           BEGIN     
                ...
                IF V_TOTAL = V_TOTAL THEN
                    ...
                    --UTL_FILE.PUT_LINE (V_MANEJADOR, 'TRADPP  '||'PAC941215E50');  --S(13)
                    
                    IF HED_REC.TIPO_ADENDA = 'WALMART' THEN
                        UTL_FILE.PUT_LINE (V_MANEJADOR, 'TRADPP  '||V_RFC);  --S(13)
                    ELSE
                        UTL_FILE.PUT_LINE (V_MANEJADOR, 'TRADPP  '||V_NAME);  --S(13)
                    END IF;
...
-----------------------17 JUN 2012 --------------- ANEXO -----------------------------
                    ...
                    --IF V_RFC = 'ROBG560202PS5' THEN
                        --UTL_FILE.PUT_LINE (V_MANEJADOR, 'REGIMEN  '||'PERSONA FISICA CON ACTIVIDAD EMPRESARIAL Y PROFESIONAL');
                    --ELSE
                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'REGIMEN  '||v_regimen);
                    --END IF;

                    IF (v_mpago = 'N' AND v_cuenta = 'N') AND V_RFC = 'RTO840921RE4' THEN
                        UTL_FILE.PUT_LINE (V_MANEJADOR, 'CTAPAG  '||'');  --N(2)
                        UTL_FILE.PUT_LINE (V_MANEJADOR, 'METPAG  '||'NA');  --S  METODO DE PAGO
                    ELSIF (v_mpago = 'N' AND v_cuenta = 'N') AND V_RFC <> 'RTO840921RE4' THEN
                        ...
                        UTL_FILE.PUT_LINE (V_MANEJADOR, 'CTAPAG  '||'');  --N(2)
                        UTL_FILE.PUT_LINE (V_MANEJADOR, 'METPAG  '||'02');  --S  METODO DE PAGO                        
                    ELSE
                        IF v_cuenta = 'N' THEN 
                            v_cuenta := ''; 
                        END IF;
...
--                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'FECEXP_ORI  '||HED_REC.FECHA_FACTURA);  --S  FECHA DE EXPEDICI?N
                    ...
                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'REFEMI  '||HED_REC.SERIE||HED_REC.FACTURA);  
                    --===============================================================================================================--
...
--                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'AOAPRO  '||HED_REC.PERIODO_APROB);  -- A?O DE APROBACI?N
                    ...
                    --IF NVL(P_EXTEMP,0) > 0 THEN
                        ...
                        UTL_FILE.PUT_LINE (V_MANEJADOR, 'TDA_CONTRA  '||V_CONTRA);  --TDA_CONTRA #folio
...
--                    END IF;
                    ...
                    --====================================================================--
                    IF HED_REC.TIPO_ADENDA = 'WALMART' THEN
                        UTL_FILE.PUT_LINE (V_MANEJADOR, 'CONTRA  '||V_CONTRA); --CONTRARECIBO HOJA CARAMELO);  --S  NUMERO DE RECEPCION DE WALMART
                    END IF;                    
                    --====================================================================--
                    ...
                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'RFCEMI  '||V_RFC);  --S  R.F.C. DEL EMISOR DE LA FACTURA        
                    ...
                    --ELSE
                        --UTL_FILE.PUT_LINE (V_MANEJADOR, 'NOMEMI  '||'PRODUCTOS AVICOLAS EL CALVARIO S. DE R.L. DE C.V.');  --S  NOMBRE DEL EMISOR DE LA FACTURA
                        UTL_FILE.PUT_LINE (V_MANEJADOR, 'NOMEMI  '||V_NAME);  --S  NOMBRE DEL EMISOR DE LA FACTURA      
                    --END IF;
                    
                    IF V_RFC = 'PAC941215E50' THEN
                        UTL_FILE.PUT_LINE (V_MANEJADOR, 'EANEMI  '||'7504003607004');  --N  EAN DEL EMISOR
                    ELSE
                        UTL_FILE.PUT_LINE (V_MANEJADOR, 'EANEMI  '||'');  --N  EAN DEL EMISOR
                    END IF;
...
--                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'CODEMI  '||'75790');  --  C?DIGO POSTAL DEL DOMICILIO FISCAL DEL EMISOR 
                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'CALEMI  '||v_street);  --S  CALLE DEL DOMICILIO FISCAL DEL EMISOR
                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'NEXEMI  '||v_extnum);  --  NUMERO EXTERIOR DEL DOMICILIO FISCAL DEL EMISOR
                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'COLEMI  '||v_neigh);  --  COLONIA DEL DOMICILIO FISCAL DEL EMISOR
                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'MUNEMI  '||v_city);  --S  MUNICIPIO DEL DOMICILIO FISCAL DEL EMISOR (DELEGACION)
                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'ESTEMI  '||v_state); --  ESTADO DEL DOMICILIO FISCAL DEL EMISOR
                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'PAIEMI  '||v_country);  --S  PA?S DEL DOMICILIO FISCAL DEL EMISOR
                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'CODEMI  '||v_postal_code);  --  C?DIGO POSTAL DEL DOMICILIO FISCAL DEL EMISOR
                    UTL_FILE.PUT_LINE (V_MANEJADOR, '');  -- L?NEA EN BLANCO
                    --HR_LOCATIONS_ALL_V
...
--                                V_UNIDAD_PDF := 'KG';
                            ...
                            IF V_UNIDAD = 'EA' AND p_org_id in (85,83) THEN 
                                V_SEC_UOM := '';
                                V_UNIDAD_PDF := 'PZ';
                                v_unidad := 'PZ';
                            END IF;
                                ...
                                ELSE
                                    ...
                                    ELSE 
                                        IF p_org_id IN (83,84,85) THEN
                                            UTL_FILE.PUT_LINE (V_MANEJADOR, 'DESCRI  '||LIN_REC.CANTIDAD_SECUNDARIA||V_SEC_UOM||LIN_REC.DESCRIPCION);  --S  DESCRIPCI?N DEL PRODUCTO O SERVICIO
                                        ELSE
                                            UTL_FILE.PUT_LINE (V_MANEJADOR, 'DESCRI  '||SUBSTR(LIN_REC.CANTIDAD_SECUNDARIA||V_SEC_UOM||LIN_REC.DESCRIPCION,1,35));  --S  DESCRIPCI?N DEL PRODUCTO O SERVICIO
                    ...
                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'IVATRA  '||XXCALV_ROUND_TWO_DECIMAL(ROUND(V_IVA_TOTAL,2)));--N  MONTO TOTAL DE IVA TRASLADADO, IVATRA1, IVATRA2, IVATRA3  
                    ...
                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'IVARET  '||ROUND(ABS(V_IVA_RET_TOTAL),2)); --N  IVA RETENIDO (LA SUMA DE IVARET_D)
                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'ISRRET  '||ROUND(ABS(V_ISR_RET_TOTAL),2)); --N  ISR RETENIDO (LA SUMA DE ISRRET_D)
                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'TOTPAG  '||XXCALV_ROUND_TWO_DECIMAL(V_TOTAL - ABS(NVL(V_IVA_RET_TOTAL,0)) - ABS(NVL(V_ISR_RET_TOTAL,0))));--N  TOTAL A PAGAR
                    UTL_FILE.PUT_LINE (V_MANEJADOR, 'TOTPAG_ORI  '||XXCALV_ROUND_TWO_DECIMAL(V_TOTAL - ABS(NVL(V_IVA_RET_TOTAL,0)) - ABS(NVL(V_ISR_RET_TOTAL,0))));--N  TOTAL A PAGAR
