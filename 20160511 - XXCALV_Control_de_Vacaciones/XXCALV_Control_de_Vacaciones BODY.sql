...
--  END;
  ...
  --
  -- Descripci€n:  Inicializa variables globales o ngenerales del PKG.
  --
      ...
      FOR v_Eventos IN c_Eventos (v_Unicos.PERSON_ID, v_Unicos.ANIO_ANTIGUEDAD) LOOP
        ...
        ELSE
          IF v_Saldo_Inicial IS NULL THEN
            errbuf  := 'Error:  Orden de eventos no v∑lido para id_persona: ' || v_Eventos.Person_Id || ' - A“o: ' || v_Eventos.Anio_Antiguedad;
            XXSTO_TOOLS_PKG.genera_salida(errbuf, 'B');
            retcode := 2;
            RETURN;
          END IF;
          ...
          ELSIF v_Eventos.ID_TIPO_EVENTO IN (6, 7, 8) THEN  -- Rechazo, Cancelaci€n y Vencimiento regresan los saldos.
            v_Nuevo_Saldo   := v_Nuevo_Saldo + v_Eventos.DIAS_EVENTO;
          ELSE -- (5, 9, 10)  -- Aprobaci€n, vacaciones disfrutadas y vacaciones pagadas, solo confirman el saldo
...
--      END IF;
  ...
  --
  -- Descripci€n:  Toma la infomaci€n de los dÃas asignados para el perÃodo actual y siguiente, para luego restar de estos
  --               el nømero de dÃas utilizados en cada uno.  De esa manera se obtiene cada disponible.
  --
  -- Par∑metros: errbuf:  Mensaje de error en caso de presentarse uno
  --             retcode: 0 = termin€ bien
  --                      1 = termin€ con advertencia
  ...
  --             p_Assignment_Id:      Id de la asignaci€n del empleado
  --             p_Payroll_Id:         Id dela n€mina del empleado
  --             p_Procesar_Fecha_Min:'S' para regresar la fecha MÃnima de vacaciones.  'N' para no hacer este prceso (consume tiempo)
  --             x_Fecha_Minima_Vac:  Basado en la øltima ejecuci€n de pago de n€mina o pago r∑pido,
  --                                  obtiene el primer dÃa disponible para solicitar vacaciones
  --
  ...
  --                      1 = termin€ con advertencia
  --                      2 = termin€ con error
  --             p_Person_Id: Id de la persona a la que se deben calcular los dÃas.
  --             p_Business_Group_Id: Id del Grupo de negocios.  (Est∑ asociado al empleado)
  --             p_Procesar_Fecha_Min:'S' para regresar la fecha MÃnima de vacaciones.  'N' para no hacer este prceso (consume tiempo)
  ...
  --             x_Dias_Actual:       Nømero de dÃas disponibles en el perÃodo actual.
  --             x_DÃas_Siguiente:    Nømero de dÃas disponibles en el siguiente perÃodo
  --             x_Registro_Base_Act: Regresa 'S' si el valor del dÃa para el perÃodo actual se tom€ de la tabla base del sistema o
  --                                          'N' si se tom€ de la tabla propia del desarrollo.
  --             x_Registro_Base_Sig: Regresa 'S' si el valor del dÃa para el perÃodo siguiente se tom€ de la tabla base del sistema o
  --                                          'N' si se tom€ de la tabla propia del desarrollo.
  --             x_Fecha_Minima_Vac:  Basado en la øltima ejecuci€n de pago de n€mina o pago r∑pido,
  --                                  obtiene el primer dÃa disponible para solicitar vacaciones
  ...
  PROCEDURE Calcula_Dias_Disponibles
    ...
    /************************************************************************************/
    /**********         MODIFICACI”N    IPONCE      30 - AGOSTO - 2016             ********/      
    /************************************************************************************/ 
  ...
  BEGIN
    ...
    IF v_Assignment_Id IS NULL THEN
      errbuf  := 'Empleado no tiene datos de asignaci€n.';
      XXSTO_TOOLS_PKG.genera_salida(errbuf, 'B');
      retcode := 2;
      RETURN;
    ELSE
      ...
      ELSE
        ...
        BEGIN
          /*******************************************************************************/
          /*****            MODIFICACI”N        05-SEPTIEMBRE-2016                    ****/
          /*******************************************************************************/
          IF QUERY_FLAG = 'Q' THEN 
              SELECT NVL(SALDO_DIAS, 0)
               ...
               WHERE 1 = 1
                 ...
                 AND ID_TIPO_EVENTO    = 1       -- Nømero de dÃas de saldo se tiene en este registro 
        ...
        EXCEPTION
          WHEN NO_DATA_FOUND THEN
            x_Dias_Actual := NULL;
            FND_FILE.PUT_LINE(FND_FILE.LOG, 'Antig¸edad : ' || p_Antiguedad_Act);
        ...
        EXCEPTION
          WHEN NO_DATA_FOUND THEN
            IF p_Valida_Informativo = 'S' THEN
              --
              errbuf  := 'El empleado a˙n no tiene el registro informativo de saldos.  Espere la ejecuci€n del concurrente nocturno.';
              XXSTO_TOOLS_PKG.genera_salida(errbuf, 'B');
              retcode := 2;
            ...
            ELSE
              --
              x_Dias_Siguiente := NULL;
              FND_FILE.PUT_LINE(FND_FILE.LOG, 'Antig¸edad Siguiente : ' || p_Antiguedad_Sig);
  ...
  --                      2 = termin€ con error
  --             p_Person_Id: Id de la persona a la que se deben calcular los dÃas.
  --             x_Numero_Empleado            Nømero de n€mina del empleado.
  ...
  --             x_Fecha_Control_Vacaciones   Fecha de la øltima revisi€n de datos del proceso de control de vacaciones.
  ...
  --             x_Dias_Actual:               Nømero de dÃas disponibles en el perÃodo actual.
  --             x_Fecha_Inicio_Act:          Fecha de inicio del perÃodo actual.
  --             x_Fecha_Fin_Act:             Fecha de finalizaci€n del perÃodo actual
  --             x_DÃas_Siguiente:            Nømero de dÃas disponibles en el siguiente perÃodo
  --             x_Fecha_Inicio_Sig:          Fecha de inicio del perÃodo siguiente.
  --             x_Fecha_Fin_Sig:             Fecha de finalizaci€n del perÃodo siguiente.
  --             x_Registro_Base_Act: Regresa 'S' si el valor del dÃa para el perÃodo actual se tom€ de la tabla base del sistema o
  --                                          'N' si se tom€ de la tabla propia del desarrollo.
  --             x_Registro_Base_Sig: Regresa 'S' si el valor del dÃa para el perÃodo siguiente se tom€ de la tabla base del sistema o
  --                                          'N' si se tom€ de la tabla propia del desarrollo.
  ...
  --
  -- Descripci€n:  Actualiza la fecha control del empleado, con la øltima fecha en que se gener€ una revisi€n y actualizaci€n de datos.
  --
  -- Par∑metros: errbuf:  Mensaje de error en caso de presentarse uno
  --             retcode: 0 = termin€ bien
  --                      1 = termin€ con advertencia
  ...
  --
  -- Descripci€n:  Actualiza el saldo de dÃas, ubicando el registro que almacena el dato en la historia.
  --
  -- Par∑metros: errbuf:  Mensaje de error en caso de presentarse uno
  --             retcode: 0 = termin€ bien
  --                      1 = termin€ con advertencia
  ...
  --            p_Dias               : Nømero de dÃas a transaccionar.  Positivo si suma al saldo;  negativo si resta del saldo.
  --
  -- Retorna el nuevo saldo disponible de dÃas.
  --
  FUNCTION Actualiza_Datos_Anio_Base
             ( errbuf                   OUT VARCHAR2
              ,retcode                  OUT NUMBER
    ...
    CURSOR c_Saldo
        ...
        SELECT XVE.ROWID                ROW_ID
         ...
         WHERE 1 = 1
           ...
           AND XVE.ID_TIPO_EVENTO      = 1                 --  DÃas asignados del periodo y ahÃ almacena saldo en dÃas
  ...
  BEGIN
    ...
    FOR v_Saldo IN c_Saldo (p_Person_Id, p_Anio_Antiguedad) LOOP
      ...
      FND_FILE.PUT_LINE(FND_FILE.LOG, 'ActualizaciÛn de saldo : ' ||  p_Dias || v_Nuevo_Saldo || ' PERSON_ID : ' || p_Person_Id);
      EXIT;
    END LOOP;
    --
  ...
  --
  -- Descripci€n:  Graba fÃsicamente la informaci€n nueva de encabezados en la tabla de historia.
  --
  -- Par∑metros: errbuf:  Mensaje de error en caso de presentarse uno
  --             retcode: 0 = termin€ bien
  --                      1 = termin€ con advertencia
  ...
  --
  -- Descripci€n:  Graba fÃsicamente la informaci€n nueva de detalle en la tabla de historia.
  --
  -- Par∑metros: errbuf:  Mensaje de error en caso de presentarse uno
  --             retcode: 0 = termin€ bien
  --                      1 = termin€ con advertencia
  ...
  --
  -- Descripci€n:  Elimina una ausencia especÃfica asociada a un empelado.
  --
  -- Par∑metros: errbuf:  Mensaje de error en caso de presentarse uno
  --             retcode: 0 = termin€ bien
  --                      1 = termin€ con advertencia
  ...
  --             p_Object_Version_Number         Nømero de versi€n del objeto a eliminar.
  --
  PROCEDURE Elimina_Ausencia
             ( errbuf                          IN OUT VARCHAR2
              ,retcode                         IN OUT NUMBER
  ...
  --
  -- Descripci€n:  Genera los movimientos de ausencia por vacaciones asociados al empleado.
  --
  -- Par∑metros: errbuf:  Mensaje de error en caso de presentarse uno
  --             retcode: 0 = termin€ bien
  --                      1 = termin€ con advertencia
  ...
  --             x_Object_Version_Number         Regresa Nømero de versi€n del objeto generado.
  ...
  BEGIN
    ...
    IF x_absence_attendance_id IS NOT NULL THEN
      ...
      IF v_Dias_Leidos <> p_Absence_Days THEN
        errbuf  := 'Los dÃas generados no corresponden a los dÃas envÃados a grabar.';
        XXSTO_TOOLS_PKG.genera_salida(errbuf, 'B');
        retcode := 2;
        RETURN;
      END IF;
  ...
  --
  -- Descripci€n:  Elimina los elemento por pago de vacaciones asociados al empleado.
  --
  -- Par∑metros: errbuf:  Mensaje de error en caso de presentarse uno
  --             retcode: 0 = termin€ bien
  --                      1 = termin€ con advertencia
  ...
  --
  -- Descripci€n:  Genera los elemento por pago de vacaciones asociados al empleado.
  --
  -- Par∑metros: errbuf:  Mensaje de error en caso de presentarse uno
  --             retcode: 0 = termin€ bien
  --                      1 = termin€ con advertencia
  ...
  --             p_Person_Id                     Id de la persona a la cual generar la ausencia
  --             p_Business_Group_Id             Business Group Id asociado al empleado
  --             p_Assignment_Id                 Id de la asignaci€n relacionada a la persona.
  --             p_Dias                          Nømero de dÃas a grabar en el elemento.
  ...
  BEGIN
    ...
    IF c_Element_Type%NOTFOUND THEN
      ...
      errbuf  := 'No se encontr€ el tipo de elemento "P037_VACACIONES P".';
      XXSTO_TOOLS_PKG.genera_salida(errbuf, 'B');
      retcode := 2;
      RETURN;
    END IF;
    ...
    IF c_Input_Values%NOTFOUND THEN
      ...
      errbuf  := 'No se encontr€ el id para la secuencia 2 del tipo de elemento "P037_VACACIONES P".';
      XXSTO_TOOLS_PKG.genera_salida(errbuf, 'B');
      retcode := 2;
      RETURN;
    END IF;
    ...
    IF v_Element_Link_Id IS NULL THEN
      errbuf  := 'No se encontr€ el LINK_ID para el elemento "P037_VACACIONES P".';
      XXSTO_TOOLS_PKG.genera_salida(errbuf, 'B');
      retcode := 2;
      RETURN;
    END IF;
  ...
  BEGIN
    ...
    IF NVL(p_Respuesta_Solicitud, 'XXX') IN ('A', 'R', 'C', 'V', 'P') THEN
      -- Respuesta del WF de aprobaci€n de vacaciones'A' € 'R'.  
      -- Cancelaci€n recibido de pantalla 'C'.
      ...
      IF c_Asignaciones%NOTFOUND THEN
        errbuf  := 'Error al obtener Asignaci€n: ' || SQLERRM;
        retcode := 2;
        ROLLBACK TO RESPUESTA;
      ...
      IF    p_Respuesta_Solicitud = 'A' THEN               -- Solicitud Aprobada
        v_xxcalv_vac_eventos.ID_TIPO_EVENTO                    := 5;    -- Aprobaci€n
      ...
      ELSIF p_Respuesta_Solicitud = 'C' THEN                 -- Solicitud Cancelada
        v_xxcalv_vac_eventos.ID_TIPO_EVENTO                    := 7;    -- Cancelaci€n
        v_xxcalv_vac_eventos.SALDO_DIAS                        := v_xxcalv_vac_evento_loc.SALDO_DIAS  + v_xxcalv_vac_evento_loc.DIAS_EVENTO;
        v_xxcalv_vac_eventos.DIAS_EVENTO                       := v_xxcalv_vac_evento_loc.DIAS_EVENTO;
        v_xxcalv_vac_eventos.DIAS_DESPLEGAR                    := v_xxcalv_vac_evento_loc.DIAS_EVENTO;
      ...
      IF SQL%NOTFOUND THEN
        errbuf  := 'Error al actualizar Evento ' || p_Event_Id || '. No se encontr€ el registro.';
        retcode := 2;
        ROLLBACK TO RESPUESTA;
        RETURN;
      END IF;
  ...
  --
  -- Descripci€n:  Genera los movimientos cuando el saldo en dÃas por disfrutar del“ periodo vencido es > 0.
  --
  -- Par∑metros: errbuf:  Mensaje de error en caso de presentarse uno
  --             retcode: 0 = termin€ bien
  --                      1 = termin€ con advertencia
  ...
  -- PROCEDURE Actualiza_Fecha_Control
  --
  -- Descripci€n:  Actualiza los registros a desplegar en la pantalla de captura de vacaciones.
  --
  -- Par∑metros: errbuf:  Mensaje de error en caso de presentarse uno
  --             retcode: 0 = termin€ bien
  --                      1 = termin€ con advertencia
  ...
  BEGIN
    ...
    FOR v_Carga IN c_Carga LOOP
      ...
      v_errbuf   := NULL;
      SAVEPOINT EMPLEADO;
      XXSTO_TOOLS_PKG.Genera_Salida('Nømero Empleado: ' || v_Carga.EMPLOYEE_NUMBER || '.  DÃas: ' || v_Carga.NUM_DIAS, 'B');
      v_Procesar := 'S';
      --
          ...
          FOR v_Historia IN c_Historia (v_Personas.PERSON_ID) LOOP
            ...
            IF v_Num_Registro = 1 THEN
              ...
              IF v_Historia.ANIO_ANTIGUEDAD <> v_Personas.ANIO_ANTIGUEDAD THEN
                v_errbuf := '  El a“o de antig∏edad actual del empleado (' || v_Personas.ANIO_ANTIGUEDAD ||
              ...
              IF v_Procesar = 'S' AND v_Carga.NUM_DIAS > v_Saldo_Actual THEN
                v_errbuf := '  El nømero de dias devengados (' || v_Carga.NUM_DIAS ||
            ...
            ELSIF v_Num_Registro = 2 THEN
              ...
              IF v_Historia.ANIO_ANTIGUEDAD <> v_Primer_Anio + 1 THEN
                v_errbuf := '  El a“o de antig∏edad siguiente del empleado (' || v_Personas.ANIO_ANTIGUEDAD ||
  ...
  --
  -- Descripci€n:  Valida los registros que deben de cambiar de estado, basado en la fecha de proceso.
  --
  -- Par∑metros: errbuf:  Mensaje de error en caso de presentarse uno
  --             retcode: 0 = termin€ bien
  --                      1 = termin€ con advertencia
  ...
  --
  -- Descripci€n:  Valida los movimientos que haya tenido un empelado desde la øltima actualizaci€n del registro.
  ...
  PROCEDURE Verifica_Cambios_Fecha
    ...
    /************************************************************************************/
    /**********         MODIFICACI”N    IPONCE      20 - MAYO - 2016             ********/      
    /************************************************************************************/             
  ...
  BEGIN
    ...
    FOR v_Personas IN c_Personas(p_person_id) LOOP
      ...
      v_retcode          := 0;
      v_Contar           := 'N';
      XXSTO_TOOLS_PKG.genera_salida('Empleado nømero: ' || v_Personas.EMPLOYEE_NUMBER, 'B');
      v_Assignment_Id := NULL;
      ...
      --
      IF v_Assignment_Id IS NULL THEN
        v_errbuf  := 'Empleado no tiene datos de asignaci€n.';
        XXSTO_TOOLS_PKG.genera_salida(v_errbuf, 'B');
        v_retcode := 2;
      ELSE
        --
        IF v_Personas.FECHA_CONTROL_VACACIONES IS NULL OR
          ...
          IF v_retcode <> 2 AND NVL(v_Dias_Asignados_Act, 0) > 0 THEN
            IF NVL(v_Registro_Base_Act, 'N') = 'S' THEN
              ...
              IF  v_Personas.FECHA_CONTROL_VACACIONES IS NULL OR v_correccion = 1 THEN     -- Solo cuando el actual no habÃa sido creado previamente
                --
