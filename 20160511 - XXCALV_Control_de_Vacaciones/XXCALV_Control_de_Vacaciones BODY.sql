...
--      END IF;
  ...
  BEGIN
    ...
    IF c_Eventos%NOTFOUND THEN
      CLOSE c_Eventos;
      errbuf  := 'No se encontr€ el registro de saldo para la antiguedad: ' || p_Anio_Antiguedad;
      XXSTO_TOOLS_PKG.genera_salida(errbuf, 'B');
      retcode := 2;
      RETURN;
    END IF;
        ...
        IF v_Personas.FECHA_CONTROL_VACACIONES IS NULL OR
          ...
          IF v_retcode <> 2 AND NVL(v_Dias_Asignados_Act, 0) > 0 THEN
            IF NVL(v_Registro_Base_Act, 'N') = 'S' THEN
              IF  v_Personas.FECHA_CONTROL_VACACIONES IS NULL  THEN     -- Solo cuando el actual no habÃa sido creado previamente
                --
