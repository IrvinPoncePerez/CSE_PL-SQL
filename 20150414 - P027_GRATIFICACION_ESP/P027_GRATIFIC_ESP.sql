...
IF  (MesPago >= 03 AND MesPago <= 04) AND DIAS >= 60 THEN
( 
...
IF  MesPago >= 05 AND DIAS >= 60  THEN 
 (
     IF antiguedad > 2 AND SalarioDiario >= SUELDO_MAX THEN						
	       (							  
            ...
            ELSE
			  (
			    IF ASG_ORG LIKE '%1101%02%' OR ASG_ORG LIKE '%1102%' OR ASG_ORG LIKE '%1103%01%' OR ASG_ORG LIKE '%1104%' THEN
				(
				   GratifMayo = (SalarioDiario * ((DIAS *15)/DiasAnio)) - PTU
				 )
                  ELSE				 
