CREATE OR REPLACE PROCEDURE APPS.PAC_PERCEP_Y_DEDUC_PRC(
...
--                           PTF.RUN_TYPE_NAME,
                           ...
                           NVL(PAC_RESULT_VALUES_PKG.GET_EXEMPT_VALUE(PAA.ASSIGNMENT_ACTION_ID,     'P039_DESPENSA',            'Pay Value',    'Futuro 1'), '0')   AS  DESPENSA_EXE,
...
BEGIN
    ...
    BEGIN
        ...
        var_data := 'EMPRESA A QUIEN SE PRESTA SERVICIOS,' ||
                    'CVE NOM,'                  ||
                    'DEPARTAMENTO,'             ||
                    'PUESTO,'                   ||
                    'NUMERO,'                   ||
                    ...
                    'ISPT A FAVOR,'             || 
                    'FINANCIAMIENTO IMSS,'      ||
                    'PERCEPCION BRUTA SIN BONO DE DESPENSA,' ||
        ...
        LOOP
            ...
            LOOP
                ...
                var_data :=    DETAIL(rowIndex).EMPRESA                 || ',' ||
                               ...
                               DETAIL(rowIndex).TIPO_A                  || ',' ||
                               DETAIL(rowIndex).TIPO_B                  || ',' ||
                               DETAIL(rowIndex).TIPO_MS                 || ',' ||
                               DETAIL(rowIndex).SUELDO_DIARIO           || ',' ||
                               DETAIL(rowIndex).SALARIO_DIARIO_INTEGRADO|| ',' ||
                               DETAIL(rowIndex).CREDITO_INFONAVIT       || ',' ||
                               DETAIL(rowIndex).FECHA_INFONAVIT         || ',' ||
                               DETAIL(rowIndex).TIPO_INFONAVIT          || ',' ||
                               DETAIL(rowIndex).VALOR_INFONAVIT         || ',' ||
                               ...
                               DETAIL(rowIndex).PERCEPCIONES_BRUTAS     || ',' ||
                               ...
                               DETAIL(rowIndex).DEDUCCIONES_BRUTAS      || ',' ||
                               DETAIL(rowIndex).NETO_A_PAGAR            || ',' ||
