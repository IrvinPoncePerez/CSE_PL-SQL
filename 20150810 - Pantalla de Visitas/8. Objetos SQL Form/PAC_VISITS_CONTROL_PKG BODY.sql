PACKAGE BODY PAC_VISITS_CONTROL_PKG IS
  ...
  BEGIN
    ...
    IF VALIDATE_DATA() = TRUE AND :PAC_VISITS_CONTROL_TB.ATTRIBUTE3 IS NULL THEN 
      ...
      :PAC_VISITS_CONTROL_TB.ATTRIBUTE5 := 0;
      :PAC_VISITS_CONTROL_TB.ATTRIBUTE6 := 0;
      :PAC_VISITS_CONTROL_TB.ATTRIBUTE7 := 0;
  ...
  PROCEDURE COUNT_PRINT(ATTRIBUTE_NUMBER NUMBER, VISITOR_ID NUMBER) IS
  BEGIN
    IF    (ATTRIBUTE_NUMBER = 5) THEN
      
      UPDATE PAC_VISITS_CONTROL_TB PVC
         SET PVC.ATTRIBUTE5 = NVL(PVC.ATTRIBUTE5, 0) + 1
       WHERE 1 = 1
         AND PVC.VISITOR_DAY_ID = VISITOR_ID;
         
    ELSIF (ATTRIBUTE_NUMBER = 6) THEN
      
      UPDATE PAC_VISITS_CONTROL_TB PVC
         SET PVC.ATTRIBUTE6 = NVL(PVC.ATTRIBUTE6, 0) + 1
       WHERE 1 = 1
         AND PVC.VISITOR_DAY_ID = VISITOR_ID;
         
    ELSIF (ATTRIBUTE_NUMBER = 7) THEN
      
      UPDATE PAC_VISITS_CONTROL_TB PVC
         SET PVC.ATTRIBUTE7 = NVL(PVC.ATTRIBUTE7, 0) + 1
       WHERE 1 = 1
         AND PVC.VISITOR_DAY_ID = VISITOR_ID;
