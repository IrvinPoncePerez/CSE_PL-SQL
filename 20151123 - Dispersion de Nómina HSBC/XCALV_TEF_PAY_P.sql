...
/******************************************************************************************
 ...
 2.0       23/11/2015 IPONCE          2. Modificación por método de pago HSBC.
******************************************************************************************/
   ...
   CURSOR c_lookup(p_lookup_type VARCHAR2, p_lookup_code VARCHAR2) IS    
    ...
    CURSOR c_time_periods (p_payroll_id         VARCHAR2,
        ...
        SELECT PTP.PERIOD_NAME
         ...
         WHERE 1 = 1
           ...
           AND PTP.PERIOD_TYPE = NVL(p_period_type, PTP.PERIOD_TYPE)
           AND PTP.START_DATE = NVL(p_start_date, PTP.START_DATE)
   ...
   --Datos hsbc
   l_indicador_formato_archivo  fnd_lookup_values.description%TYPE;
   l_nivel_autorizacion         fnd_lookup_values.description%TYPE;
   l_cuenta_cargo               fnd_lookup_values.description%TYPE;
   l_referencia_lote            fnd_lookup_values.description%TYPE;
   l_hora_aplicacion            fnd_lookup_values.description%TYPE;
    ...
    FOR l_emp IN c_emp(l_start_date,l_end_date) LOOP
        ...
        IF l_pago.banco like '%BANCOMER%' THEN
            ...
            IF mod(l_count_bbva,l_split) = 1 THEN
                print_out('***** ARCHIVOS DE DISPERSION BANCARIA '||sysdate||' *****');  
                print_out(' ');  
                ...
                print_out('* ARCHIVO DE DISPERSION BANCOMER: '||l_fname_bbva);    
        ...
        ELSIF l_pago.banco like '%BANORTE%'  THEN
            ...
            IF l_count_banorte = 1 THEN
                ...
                l_file_banorte := UTL_FILE.fopen ( l_dir, l_fname_banorte, 'w' );
                 print_out('* ARCHIVO DE DISPERSION BANORTE: '||l_fname_banorte);
        ...
        ELSIF l_pago.banco like '%HSBC%' THEN
        
            l_count_hsbc := l_count_hsbc + 1;    

            --Abrir archivo nuevo
            IF l_count_hsbc = 1 THEN
            
                OPEN c_lookup('XX_DATOS_HSBC','INDICADOR_FORMATO_ARCHIVO');
                FETCH c_lookup INTO l_indicador_formato_archivo; 
                CLOSE c_lookup;
                
                OPEN c_lookup('XX_DATOS_HSBC','NIVEL_AUTORIZACION');
                FETCH c_lookup INTO l_nivel_autorizacion; 
                CLOSE c_lookup;
        
                OPEN c_lookup('XX_DATOS_HSBC','CUENTA_CARGO');
                FETCH c_lookup INTO l_cuenta_cargo; 
                CLOSE c_lookup;
                
                OPEN c_lookup('XX_DATOS_HSBC','REFERENCIA_LOTE');
                FETCH c_lookup INTO l_referencia_lote; 
                CLOSE c_lookup;
                
                OPEN c_lookup('XX_DATOS_HSBC','HORA_APLICACION');
                FETCH c_lookup INTO l_hora_aplicacion; 
                CLOSE c_lookup; 
                
                OPEN c_time_periods (p_payroll_id,
                                     p_consolidation_id,
                                     p_period_type,
                                     l_start_date,
                                     l_end_date);
                FETCH c_time_periods INTO l_period_name;
                CLOSE c_time_periods;
                
                l_linea := l_indicador_formato_archivo || ',' ||
                           l_nivel_autorizacion || ',' ||
                           lpad(l_cuenta_cargo,10,'0') || ',' ||
                           l_emp.importe_total || ',' ||
                           l_emp.empleados || ',' ||
                           to_char(NVL(l_fecha_aplicacion, l_end_date),'DDMMYYYY') || ',' ||
                           l_hora_aplicacion || ',' ||
                           UPPER (l_referencia_lote || ' ' || l_period_name);
            ...
            l_linea := lpad(l_pago.cuenta,10,'0') || ',' ||
                       l_emp.importe || ',' ||
                       UPPER (l_referencia_lote || ' ' || l_period_name) || ',' ||
                       l_emp.full_name;          
